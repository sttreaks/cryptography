...
def generateAES256():
    np.random.seed(0)
    W = [np.random.randint(0, 256) for _ in range(16)]
    bd = Sha256(W, gen=True)
...
def get_file_hash(fname):
    with open(fname, "rb") as f:
        data = f.read()
    bd = Sha256(data, gen=True)
    return ''.join('{:02x}'.format(i) for i in bd)


def check_file_hash(fname, hash):
    if get_file_hash(fname) == hash:
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    # print(''.join('{:02x}'.format(i) for i in bd))
    # generateAES256()
    print(get_file_hash("/Users/a1exandr0/PycharmProjects/sha256/basophil_1_augment_n_1.png"))
    hash = get_file_hash("/Users/a1exandr0/PycharmProjects/sha256/basophil_1_augment_n_1.png")
    print(check_file_hash("/Users/a1exandr0/PycharmProjects/sha256/test.txt", hash))
